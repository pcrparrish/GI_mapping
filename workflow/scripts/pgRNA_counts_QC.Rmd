---
title: "CRISPR pipeline: gRNA counts QC"
...
## To Do
* add a step to define ordering of samples => convert sample variable into factor
...
in_dir <- file.path("/Volumes", "Projects", "paralog_pgRNA", "pgPEN_library", "GI_mapping")
...
d.counts <- read_tsv(file.path(in_dir, "results", "pgRNA_counts", "PP_pgPEN_HeLa_counts.txt"), col_names=TRUE)
#d.counts
...
ggplot(d.counts_cdf, aes(x = count_norm, color = sample)) +
  stat_ecdf() +
...
ggplot(d.counts_cpm, aes(x = log2_cpm, fill = sample)) +
...
cor_heatmap_unfiltered <- pheatmap(d.counts_cpm_cor,
                 col = colors,
                 border_color = "white",
...
## filter out pgRNAs with low plasmid read counts
...
plot.plasmid_cpm <-  ggplot(d.counts_cpm_plasmid, aes(x = log2_cpm)) +
...
d.counts_cpm_plasmid_stats %>% kbl %>% kable_styling(full_width = FALSE, position = "left")
...
## add counts back in, filter out pgRNAs, write output
d.counts_cpm_filtered <- d.counts_cpm_filter %>%
  filter(rm_pgRNA == FALSE)
d.counts_cpm_filtered

## also save relevant plots
